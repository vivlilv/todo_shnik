...
function Task({ task, func_delete_task, func_edit_task }) {
    ...
    useEffect(()=>{
        const savedPosition = JSON.parse(localStorage.getItem(`task-${task.id}-position`))
        if (savedPosition) {
            setTaskPosition(savedPosition)
        }
    }, [task.id])

    const handleDragStop = (e, data) => {
        setTaskPosition({ x: data.x, y: data.y });
        localStorage.setItem(`task-${task.id}-position`, JSON.stringify({ x: data.x, y: data.y }));
      };
...
//bounds property can be added
//onDrag effect can be added
    return (
        <Draggable position={taskPosition} onStop={handleDragStop} >
        <div className="p-4 border border-gray-300 rounded mb-4 bg-gradient-to-br from-cyan-500 to-blue-500 ">
            {renderTaskContent()}
        </div>
        </Draggable>
    );
